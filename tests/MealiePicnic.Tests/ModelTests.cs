...
{
    private static ShoppingItem Item(double quantity) => new(
        ItemId: "i1", FoodId: "f1", Display: "melk", FoodName: "melk",
        Quantity: quantity, Label: "Zuivel", Checked: false,
        State: LinkState.Linked, PicnicUid: "s1", PicnicLabel: null);
    ...
    public void Amount_never_drops_below_one(double quantity, int expected) =>
        Assert.Equal(expected, Item(quantity).Amount);
